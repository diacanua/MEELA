	      ...
	      else
	      {
	        ...
	        if($isUploadSuccess)
	        {
			  ...
			  if ($isSuccess && $isUploadSuccess)
			  {
					...
					$imagem =  verifyInput($_FILES['imagem']['name']);
	        $db = Database::connect();
	        $statement = $db->prepare("INSERT INTO galeria (titre,descricao_curta,descricao_longa,image,id_area, tag_desc) VALUES(?, ?,?, ?, ?,?) ");
	        $statement->execute(array($titulo,$desc_curta,$desc_longa,$imagem,$id_area,$desc_tag));
					var_dump($isSuccess);
					//Tratamento tag
					$tab_name='galeria';
